...
export const usePlaybackSnapshot = () => useMessageSnapshot().playback;
...
export const useMessageActions = () => {
  ...
  const setPlayback = useCallback((status: Partial<MessageSnapshot['playback']>) => {
    messageBus.setPlayback(status);
  }, []);

  return { appendUserMessage, appendAssistantMessage, updateMessage, setVoiceMetadata, setPlayback };
};
